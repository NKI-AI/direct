...
class Conv3dGRU(nn.Module):
    """3D Convolutional GRU Network."""
    ...
    def __init__(
        ...
        for idx in range(num_layers + 1):
            ...
            if replication_padding:
                if idx == 1:
                    block.append(nn.ReplicationPad3d(2))
                else:
                    block.append(nn.ReplicationPad3d(2 if idx == 0 else 1))
            block.append(
                nn.Conv3d(
                    in_channels=in_ch,
                    out_channels=out_ch,
        ...
        for idx in range(num_layers):
            for gru_part in [self.reset_gates, self.update_gates, self.out_gates]:
                gru_block: List[nn.Module] = []
                if instance_norm:
                    gru_block.append(nn.InstanceNorm3d(2 * hidden_channels))
                gru_block.append(
                    nn.Conv3d(
                        in_channels=2 * hidden_channels,
                        out_channels=hidden_channels,
    ...
    def forward(
        ...
        """Computes Conv3dGRU forward pass given tensors `cell_input` and `previous_state`.
        ...
        if previous_state is None:
            batch_size, spatial_size = cell_input.size(0), (cell_input.size(2), cell_input.size(3), cell_input.size(4))
            state_size = [batch_size, self.hidden_channels] + list(spatial_size) + [self.num_layers]
            previous_state = torch.zeros(*state_size, dtype=cell_input.dtype).to(cell_input.device)
...
class NormConv3dGRU(nn.Module):
    """Normalized 3D Convolutional GRU Network.
    ...
    def unnorm(input_data: torch.Tensor, mean: torch.Tensor, std: torch.Tensor, num_groups: int) -> torch.Tensor:
        b, c, d, h, w = input_data.shape
        input_data = input_data.reshape(b, num_groups, -1)
        return (input_data * std + mean).reshape(b, c, d, h, w)
    ...
    def forward(
        ...
        """Computes :class:`NormConv3dGRU` forward pass given tensors `cell_input` and `previous_state`.

        It performs group normalization on the input before the forward pass to the :class:`Conv3dGRU`.
        Output of :class:`Conv3dGRU` is then un-normalized.
