...
More specifically, given multi-coil ($n_c$) sub-sampled $k$-space measurements $\displaystyle \vec{y} \, = \, \displaystyle \big\{(\vec{y}_1, \, ...,\, \vec{y}_{n_c}} \big\}  \, = \, \big\{ U \mathcal{F} \displaystyle ( S_{i} \vec{x} ) \big\}_{i=1}^{n_{c}}$ as input, these models aim to predict the reconstructed image $\vec{x}$. The corresponding Inverse problem replaces \eqref{eq:eq2} with the following form:
