...
class EndToEndVarNetKSpace3DEngine(MRIModelEngine):
    """End-to-End Variational Network Engine for 3D k-space data.
    ...
    **models: nn.Module
    ...
    def __init__(
        ...
        **models: nn.Module,
        ...
        """Inits :class:`EndToEndVarNetKSpace3DEngine`.
    ...
    def forward_function(self, data: dict[str, Any]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        output_kspace = data["masked_kspace"] + T.apply_mask(
            T.apply_padding(output_kspace, padding=data.get("padding", None)),
            ~data["sampling_mask"],
            return_mask=False,
        )
        output_image = T.reduce_operator(
            self.backward_operator(output_kspace, dim=self._spatial_dims), data["sensitivity_map"], dim=self._coil_dim
        )
