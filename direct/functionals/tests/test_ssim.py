...
def test_ssim():
    ...
    for sigma in range(0, 101, 20):
        ...
        for _ in range(N_repeat):
            ssim_skimage = structural_similarity(image, image_noise, win_size=11, multichannel=True,
                                                 sigma=1.5, data_range=255, use_sample_covariance=False,
                                                 gaussian_weights=True)

        image_torch = torch.from_numpy(image).unsqueeze(0).permute(0, 3, 1, 2)  # 1, C, H, W
        image_noise_torch = torch.from_numpy(image_noise).unsqueeze(0).permute(0, 3, 1, 2)

        image_batch.append(image_torch)
        image_noise_batch.append(image_noise_torch)

        for _ in range(N_repeat):
            ssim_torch = batch_ssim(image_noise_torch, image_torch, win_size=11, data_range=255)
    ...
    ssim_batch = batch_ssim(image_noise_batch, image_batch, win_size=11, data_range=255, reduction='none')
