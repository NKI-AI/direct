...
class Checkpointer:
    def __init__(self, model: torch.nn.Module, save_directory: pathlib.Path,
                 save_to_disk: bool = True, **checkpointables: Any):
        ...
        if hasattr(model, 'module'):
            if not isinstance(model, (DistributedDataParallel, DataParallel)):
                self.logger.warning(f'Model has a `.module` property and is not derived from DistributeDataParallel'
    ...
    def load(self, iteration: Optional[Union[int, str, type]],
             checkpointable_objects: Optional[Any] = None) -> Dict:
        ...
        if iteration == 'latest':
            ...
            if last_model_text_path.exists():
                ...
                self.logger.info(f'Latest model not found. Perhaps `last_model.txt` (path = {last_model_text_path}) '
