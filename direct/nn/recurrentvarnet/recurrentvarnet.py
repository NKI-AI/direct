...
from direct.nn.recurrent.recurrent import Conv2dGRU, Conv3dGRU, NormConv2dGRU, NormConv3dGRU
...
class RecurrentInit3d(nn.Module):
    """Recurrent State Initializer (RSI) module of 3D Recurrent Variational Network as presented in [1]_.
...
class RecurrentVarNet3dBlock(nn.Module):
    r"""3D Recurrent Variational Network Block :math:`\mathcal{H}_{\theta_{t}}` as presented in [1]_.
    ...
    def __init__(
        ...
        # Recurrent Unit of RecurrentVarNet Block :math:`\mathcal{H}_{\theta_t}`
        self.regularizer = (
            NormConv3dGRU(**regularizer_params) if normalized else Conv3dGRU(**regularizer_params)  # type: ignore
        )
    ...
    def forward(
        ...
        hidden_state: Union[None, torch.Tensor],
        coil_dim: int = 1,
        spatial_dims: Tuple[int, int] = (3, 4),
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Computes forward pass of RecurrentVarNet3dBlock.

        Parameters
        ----------
        current_kspace: torch.Tensor
            Current k-space prediction of shape (N, coil, depth, height, width, complex=2).
        masked_kspace: torch.Tensor
            Masked k-space of shape (N, coil, depth, height, width, complex=2).
        sampling_mask: torch.Tensor
            Sampling mask of shape (N, 1, 1 or depth, height, width, 1).
        sensitivity_map: torch.Tensor
            Coil sensitivities of shape (N, coil, depth, height, width, complex=2).
        hidden_state: torch.Tensor, Optional
            Recurrent unit hidden state of shape (N, hidden_channels, depth, height, width, num_layers) if not None.
        coil_dim: int
            Coil dimension. Default: 1.
        spatial_dims: tuple of ints
            Spatial dimensions. Default: (3, 4).

        Returns
        -------
        new_kspace: torch.Tensor
            New k-space prediction of shape (N, coil, depth, height, width, complex=2).
        hidden_state: torch.Tensor
            Next hidden state of shape (N, hidden_channels, depth, height, width, num_layers).
        """
        ...
        recurrent_term, hidden_state = self.regularizer(recurrent_term, hidden_state)  # :math:`w_t`, :math:`h_{t+1}`
        recurrent_term = recurrent_term.permute(0, 2, 3, 4, 1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            self.initializer_initialization = initializer_initialization
            self.initializer = RecurrentInit3d(
                in_channels,
                recurrent_hidden_channels,
                channels=initializer_channels,
                dilations=initializer_dilations,
    ...
    def forward(
        ...
        **kwargs,
        ...
        masked_kspace: torch.Tensor
            Masked k-space of shape (N, coil, depth, height, width, complex=2).
        sampling_mask: torch.Tensor
            Sampling mask of shape (N, 1, depth, height, width, 1).
        sensitivity_map: torch.Tensor
            Coil sensitivities of shape (N, coil, depth, height, width, complex=2).
