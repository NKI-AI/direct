...
class CIRIMEngine(Engine):
    ...
    def _do_iteration(
        ...
        with autocast(enabled=self.mixed_precision):
            ...
            for key, value in regularizer_dict.items():
                regularizer_dict[key] = value + sum(
                    self.process_loss(regularizer_fns[key], output_image=output_image, **data)
                )
        ...
        output_image = output_image[-1][-1]  # Output image of the last iteration of the last cascade
    ...
    @staticmethod
    def process_loss(loss_fn, reduction, output_image, **data) -> Generator:
        """
        ...
        -----------
            ...
            output_image:  shape (batch, complex=2, height, width)
            data:  contains key "target" with value a tensor of shape (batch, height, width)

        Returns:
        --------
            Generator of loss values.
        """

        cascades = len(output_image)
        time_steps = len(output_image[0])

        # Weights the loss by the number of time steps
        weighted_time_steps = torch.logspace(-1, 0, steps=time_steps)

        weighted_cascades_loss = []
        for cascade_output in output_image:
            time_steps_loss = []
            for time_step_output in cascade_output:

                if reduction == "mean":
                    time_steps_loss.append(loss_fn(time_step_output, **data, reduction="mean"))
                else:
                    time_steps_loss.append(loss_fn(time_step_output, **data))

            weighted_time_steps_loss = [
                time_step_loss * weighted_time_steps[i] for i, time_step_loss in enumerate(time_steps_loss)
            ]
            weighted_cascades_loss.append(sum(weighted_time_steps_loss) / time_steps)

        yield sum(weighted_cascades_loss) / cascades
                ...
                for condition in reconstruction_conditions:
                    if condition:
                        ...
                        if is_validation_process:
                            ...
                            volume = volume / torch.max(torch.abs(volume))
