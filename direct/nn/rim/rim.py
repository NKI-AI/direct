...
class ConvGRUCell(nn.Module):
    ...
    def __init__(self, x_channels: int, hidden_channels, depth=2, gru_kernel_size=1, ortho_init=True,
                 instance_norm=False, dense_connection=0, replication_padding=False):
        super().__init__()
        self.depth = depth
        ...
        for idx in range(depth + 1):
            ...
            block.append(nn.Conv2d(in_ch, out_ch, 5 if idx == 0 else 3, dilation=(2 if idx == 1 else 1), padding=pad))
        ...
        for idx in range(depth):
            for gru_part in [self.reset_gates, self.update_gates, self.out_gates]:
                ...
                block.append(nn.Conv2d(2 * hidden_channels, hidden_channels, gru_kernel_size, padding=gru_kernel_size // 2))
        ...
        if ortho_init:
            for reset_gate, update_gate, out_gate in zip(self.reset_gates, self.update_gates, self.out_gates):
                ...
                nn.init.constant_(reset_gate[-1].bias, -1.)
                nn.init.constant_(update_gate[-1].bias, 0.)
                nn.init.constant_(out_gate[-1].bias, 0.)
    ...
    def forward(self, cell_input, previous_state):
        ...
        for idx in range(self.depth):
            ...
            delta = torch.tanh(self.out_gates[idx](
...
class RIM(nn.Module):
    def __init__(self, x_channels: int, num_hidden_channels: int, grad_likelihood: nn.Module, length: int = 8,
                 depth: int = 1, no_sharing: bool = True, instance_norm: bool = False, dense_connection: bool = False,
        ...
        for _ in range(length if no_sharing else 1):
            self.cell_list.append(ConvGRUCell(
                x_channels, num_hidden_channels, depth=depth, instance_norm=instance_norm,
                dense_connection=dense_connection, replication_padding=replication_padding))
        self.length = length
    ...
    def forward(self,
        ...
        for cell_idx in range(self.length):
            ...
            grad_loglikelihood = self.grad_likelihood(
                intermediate_image, masked_kspace, sensitivity_map, sampling_mask, loglikelihood_scaling)

            if grad_loglikelihood.abs().max() > 150.0:
                warnings.warn(f'Very large values for the gradient loglikelihood ({grad_loglikelihood.abs().max()}). '
                              f'Might cause difficulties.')

            cell_input = torch.cat([intermediate_image.rename(None), grad_loglikelihood.rename(None)], dim=1)
