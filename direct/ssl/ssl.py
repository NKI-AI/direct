...
class SSLTransformMaskPrefixes(DirectEnum):
    r"""SSL Transform mask prefixes.

    These are used to prefix the input (:math:`\Theta) and target (:math:`\Lambda`) k-spaces/masks in the sample.
    """
...
class MaskSplitter(DirectModule):
    ...
    def __init__(
        ...
        ratio: float | list[float] | tuple[float, ...] = 0.5,
        acs_region: list[int] | tuple[int, int] = (0, 0),
        keep_acs: bool = False,
        use_seed: bool = True,
        ...
        self.ratio: list[float] | tuple[float, ...] = ratio  # ty: ignore[invalid-assignment]
    ...
    def _gaussian_split(
        ...
        mask: torch.Tensor,
        std_scale: float = 3.0,
        seed: tuple[int, ...] | list[int] | int | Iterable[int] | None = None,
    ...
    def _half_split(
        self, mask: torch.Tensor, direction: HalfSplitType, acs_mask: torch.Tensor | None = None
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if direction in [HalfSplitType.HORIZONTAL, HalfSplitType.VERTICAL]:
            ...
            if direction == HalfSplitType.DIAGONAL_RIGHT:
    ...
    @abstractmethod
    def split_method(
        self, sampling_mask: torch.Tensor, acs_mask: torch.Tensor | None, seed: int | Iterable[int] | None
    ) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, sample: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            The updated sample with the two disjoint masks, input_mask and target_mask, as well as the two disjoint k-spaces.
        """
