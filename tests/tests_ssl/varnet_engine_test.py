...
def test_varnet_ssl_engine(shape, loss_fns, num_layers, num_filters, num_pull_layers):
    # Operators
...
def test_varnet_jssl_engine(shape, loss_fns, num_layers, num_filters, num_pull_layers):
    # Operators
    forward_operator = functools.partial(fft2, centered=True)
    backward_operator = functools.partial(ifft2, centered=True)
    # Models
